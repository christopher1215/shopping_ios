...
@interface FDTakeController() <UIImagePickerControllerDelegate, UIActionSheetDelegate, UIAlertViewDelegate, UINavigationControllerDelegate,ELCImagePickerControllerDelegate>
...
//            if([title isEqualToString:[self textForButtonWithTitle:kTakePhotoKey]])
//            {
                self.imagePicker.sourceType = [[self.sources objectAtIndex:buttonIndex] integerValue];
...
//                [[self presentingViewController] presentViewController:elcPicker animated:YES completion:nil];
//            }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info          //自拍的事后发生的函数
{
...
-(void)elcImagePickerController:(ELCImagePickerController *)picker didFinishPickingMediaWithInfo:(NSArray *)infos       //从相册选择图片的事后发生的函数
{
    for(NSDictionary *info in infos)
    {
        if ([[info objectForKey:UIImagePickerControllerMediaType] isEqualToString:@"ALAssetTypePhoto"]){
            if ([info objectForKey:UIImagePickerControllerOriginalImage]){
                ...
                if ([self.delegate respondsToSelector:@selector(takeController:gotPhoto:withInfo:)])
                    [self.delegate takeController:self gotPhoto:image withInfo:info];
            }
        }
    }
    if ([self respondsToSelector:@selector(dismissViewControllerAnimated:completion:)])
